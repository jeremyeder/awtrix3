...
- Exception: the native `Weather` app intentionally runs on-device and fetches Open-Meteo directly because it must keep working without a companion computer. Its coordinates live in `/DoNotTouch.json`, not in `/api/settings` or source control.
